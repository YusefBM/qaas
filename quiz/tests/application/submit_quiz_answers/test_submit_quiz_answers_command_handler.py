...
class TestSubmitQuizAnswersCommandHandler(unittest.TestCase):
    ...
    def test_handle_success(self, mock_transaction):
        ...
        self.user_repository_mock.find_or_fail_by_id.return_value = mock_participant
        ...
        self.user_repository_mock.find_or_fail_by_id.assert_called_once_with(
    ...
    def test_handle_quiz_not_found_raises_exception(self, mock_transaction):
        ...
        self.user_repository_mock.find_or_fail_by_id.assert_not_called()
        self.participation_repository_mock.save.assert_not_called()

    @patch("quiz.application.submit_quiz_answers.submit_quiz_answers_command_handler.transaction")
    def test_handle_user_not_found_raises_exception(self, mock_transaction):
        mock_quiz = Mock(spec=Quiz)
        ...
        self.quiz_repository_mock.find_or_fail_by_id.return_value = mock_quiz
        self.user_repository_mock.find_or_fail_by_id.side_effect = UserNotFoundException(
    ...
    def test_handle_participation_not_found_raises_exception(self, mock_transaction):
        ...
        self.participation_repository_mock.find_or_fail_by_quiz_and_participant.side_effect = (
            ParticipationNotFoundException(self.participation_id)
        )

        with self.assertRaises(ParticipationNotFoundException):
            self.handler.handle(self.command)
    ...
    def test_handle_quiz_already_completed_raises_exception(self, mock_transaction):
        ...
        with self.assertRaises(QuizAlreadyCompletedException) as context:
            self.handler.handle(self.command)

        self.assertEqual(context.exception.quiz_id, UUID("12345678-1234-5678-9abc-123456789abc"))
        self.quiz_score_calculator_mock.calculate.assert_not_called()
        self.participation_repository_mock.save.assert_not_called()

    @patch("quiz.application.submit_quiz_answers.submit_quiz_answers_command_handler.transaction")
    def test_handle_incomplete_submission_raises_exception(self, mock_transaction):
        mock_quiz = Mock(spec=Quiz)
        ...
        with self.assertRaises(IncompleteQuizSubmissionException) as context:
            self.handler.handle(self.command)

        self.assertEqual(context.exception.expected_answers, 5)
        self.assertEqual(context.exception.received_answers, 2)
        self.quiz_score_calculator_mock.calculate.assert_not_called()
        self.participation_repository_mock.save.assert_not_called()
